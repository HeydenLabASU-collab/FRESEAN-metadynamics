...
## 09-DCCM/run.sh
Compute Dynamic Cross-Correlation Matrix (DCCM) as a weighted average over all replicas `07-metadyn/run.sh`. Then analyze the DCCM to extract distances between collectively moving domains and generate a PLUMED input file to compute free energy surfaces with proper reweighting.
